...
{ 
  ...
  sortBut1 = new QRadioButton("alphabetical");
  ...
  sortButO1 = new QRadioButton("alphabetical");
  ...
  QObject::connect(sortButO1, SIGNAL(toggled(bool)), this, SLOT(sortButClickedOtherOut(bool))); // *** need to create diff slot 
  QObject::connect(sortButO2, SIGNAL(toggled(bool)), this, SLOT(sortButClickedOtherOut(bool))); // to change diff listwidget
  ...
  sortButI1 = new QRadioButton("alphabetical");
...
Multiwin::~Multiwin() {
  ...
  vector<WebPage*>::iterator it2;
  for (it2 = filenames.begin(); it2 != filenames.end(); ++it2) {
    delete *it2;
  }
  for (it2 = filenamesIn.begin(); it2 != filenamesIn.end(); ++it2) {
    delete *it2;
  }
  for (it2 = filenamesOut.begin(); it2 != filenamesOut.end(); ++it2) {
    delete *it2;
  }
...
void Multiwin::search() { 
  ...
  myEngine->add_parse_from_index_file(index, parser);
  matchingPages.clear();

  // if user left input blank
  if (wordInput->text().isEmpty()) return;

  // clear filename vector and displayed list (of matching pages)
...
void Multiwin::displayPage() { 
  ...
  set<WebPage*>::iterator it;
  MySetWebPage inPages = page->incoming_links();
  ...
  MySetWebPage outPages = page->outgoing_links();
